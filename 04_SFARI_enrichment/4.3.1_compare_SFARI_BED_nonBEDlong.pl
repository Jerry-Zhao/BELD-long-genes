#! /usr/local/perl -w
#Total-SFARI: 791	 Total-mouse: 37362	 Total-mouse-unique-symbols: 37201	 SFARI-Mouse-both-exists: 768	 SFARI-no-mouse-gene: 23
#BED-gene:         185	SFARI-overlap-BED-gene:        51
#nonBEDTotal-gene:         185	SFARI-overlap-nonBEDlong-gene:        31

#fisher.test(matrix(c(51,185-51,31,185-31),ncol=2),alternative="greater") # p-value = 0.008533


## Compare SFARI genes with long short genes
...
open(INB,"SFARI_791genes_20160126.csv")||die("Can't open INF:$!\n");
...
while(<INC>)
{
 ...
 {
  $a1++;
  if(exists $sfari{$2}){$a2++;}# print OUC "$1$2$3\t$sfari{$2}\n"; print OUCB "$1$2$3\tBED\n";}
 }
 else{print "error1\t$_\n";}
}
...
while(<IND>)
{
 ...
 {
  $a3++;
  if(exists $sfari{$2}){$a4++;}# print OUD "$1$2$3\t$sfari{$2}\n"; print OUDB "$1$2$3\tnonBEDlong\n";}
 }
 else{print "error1\t$_\n";}
}
...
print "Total-SFARI: $sfari_gene\t Total-mouse: $mouse_gene\t Total-mouse-unique-symbols: $mouse_gene2\t SFARI-Mouse-both-exists: $sfari_mouse\t SFARI-no-mouse-gene: $sfari_nomouse\n";
print "BED-gene:         $a1\tSFARI-overlap-BED-gene:        $a2\n";
print "nonBEDTotal-gene:         $a3\tSFARI-overlap-nonBEDlong-gene:        $a4\n";
