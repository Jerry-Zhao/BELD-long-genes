#! /usr/local/perl -w
#Total-Samocha: 1003	 Total-mouse: 37362	 Total-mouse-unique-symbols: 37201	 Samocha-Mouse-both-exists: 897	 Samocha-no-mouse-gene: 106
#BED-gene:         185	Samocha-overlap-BED-gene:        42
#nonBEDlong-gene:         4905	Samocha-overlap-nonBEDlong-gene:        274

# fisher.test(matrix(c(42,185-42,274,4905-274),ncol=2),alternative="greater") # p-value = 3.966e-14
...
open(INB,"Highly_constrained_genes_1003_Samocha.txt")||die("Can't open INF:$!\n");
open(INC,"../02_BELD/BED_BED_Data_length_PI30_ChIP_RPKM_cortex_WT.xls")||die("Can't open INC:$!\n");
open(IND,"../02_BELD/BED_nonBEDTotal_Data_length_PI30_ChIP_RPKM_cortex_WT.xls")||die("Can't open IND:$!\n"); 
 
#open(OUC,">SFARI_Total_overlap_all_cortex_BED_genes.xls")||die("Can't write OUA:$!\n");
#open(OUD,">SFARI_Total_overlap_all_cortex_nonBEDTotal_genes.xls")||die("Can't write OUA:$!\n");
#open(OUCB,">SFARI_Total_overlap_all_cortex_BED_genes2.xls")||die("Can't write OUA:$!\n");
#open(OUDB,">SFARI_Total_overlap_all_cortex_nonBEDTotal_genes2.xls")||die("Can't write OUA:$!\n");
...
while(<INC>)
{
 ...
 if(/^(chr\w\w?\:\d+\-\d+\:ENSMUSG\d+\_)([\w\.\-\(\)]+)(\:\w+\:[+|-])\t/)
 {
  ...
  {
   $a2++; 
  # print OUC "$1$2$3\t$sfari{$2}\n"; print OUCB "$1$2$3\tBED\n";
  }
 }
 else{print "error1\t$_\n";}
}
...
while(<IND>)
{
 ...
 if(/^(chr\w\w?\:\d+\-\d+\:ENSMUSG\d+\_)([\w\.\-\(\)]+)(\:\w+\:[+|-])\t/)
 {
  ...
  {
   $a4++; 
  # print OUD "$1$2$3\t$sfari{$2}\n"; print OUDB "$1$2$3\tnonBEDlong\n";
  }
 }
 else{print "error1\t$_\n";}
}
...
print "Total-Samocha: $sfari_gene\t Total-mouse: $mouse_gene\t Total-mouse-unique-symbols: $mouse_gene2\t Samocha-Mouse-both-exists: $sfari_mouse\t Samocha-no-mouse-gene: $sfari_nomouse\n";
print "BED-gene:         $a1\tSamocha-overlap-BED-gene:        $a2\n";
print "nonBEDlong-gene:         $a3\tSamocha-overlap-nonBEDlong-gene:        $a4\n";
