...
open(OUA,">WT_Nex_p_hmC_5coverage_allgene_body_100k_CG.txt")||die("Can't write OUB:$!\n");
...
open(INA,"/media/Project/mC/results/02_mC_hmC/calling_FDR/split/WT_Nex_p_hmC_5coverage.xls.$chromosome")||die("Can't open INC:$!\n");
open(INB,"/media/RNA/Genome/Ensembl/exon_intron/Mus_musculus.GRCm38.75.gtf.gene")||die("Can't open IND:$!\n");
...
while(<INA>)
{
 ...
 if(/^(chr\w\w?)\t(\d+)\t\-?([\d\.]+)\t(\w+)$/)
 {
  if($4 eq "CpG")
  {
   ...
   else{$a1++; $methy{$2}=$3;} ## save the methylated cytosines
  }
  elsif(($4 eq "CHG") or ($4 eq "CHH")){}
...
close OUA;
print "Total-WT_Nex_p_hmC_5coverage: $a1\tTotal-gene: $a2\n";
#}
